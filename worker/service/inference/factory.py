...
from worker.service.inference import context, error, schema
from worker.service.inference.config import get_inference_schema_files, get_inference_spec_files
from worker.service.hardware import check_metal, check_nvidia
...
def assemble_command(model_input: str, store_path: Path, options: dict[str, Any]) -> list[str]:
    cmd_factory = CommandFactory(get_inference_spec_files(), get_inference_schema_files())
    runtime = options.get("runtime", "llama.cpp")

    args_ctx = context.RamalamaArgsContext.from_dict(options)
    model_ctx = context.RamalamaModelContext(store_path, model_input)
    host_ctx = context.RamalamaHostContext(
        is_container=False,
        uses_nvidia=check_nvidia() is None,
        uses_metal=check_metal(),
        should_colorize=False,
        rpc_nodes=os.getenv("RAMALAMA_LLAMACPP_RPC_NODES", None),
    )

    ctx = context.RamalamaCommandContext(args_ctx, model_ctx, host_ctx)
    return cmd_factory.create(runtime, "serve", ctx)
